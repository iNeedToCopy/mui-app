...
var container = new Vue({
	...
	data:{
		isloading: false,
		selected_fee: 0,//选中的商品邮费
		selected_price: 0,//选中的商品价格
		selected_count: 0,//选中的商品数量
		selected_all:false,//是否全选
		car_list:[],//购物车列表
		random_goods:[],//随便逛逛
	},
	methods:{
		goto_detail:function(id){//跳转详情
			var THIS = this;
            ...
            webview.addEventListener('loaded',function(){
				mui.fire(webview,"send_id",{
					...
					cid: THIS.current_cid,
				});
			});
		},
		reset:function(){
			container.selected_fee = 0;//选中的商品邮费
			container.selected_price = 0;//选中的商品价格
			container.selected_count = 0;//选中的商品数量
			container.selected_all = false;//是否全选
			container.car_list = [];//购物车列表
			scrollTo(0,0);
		},
		get_carlist:function(){//获取订单列表
			var THIS = this;
			app.http_get('/mall/shopping/cart/home',function(res){
				if(res.code == 200){
		...
		get_random:function(){//随便逛逛
			var THIS = this;
			app.http_get('/mall/random/goods',function(res){
				app.json(res);
				THIS.random_goods = res.data.data;
			})
		},
		settle:function(){//结算购物车
			var updatestr = {
				updatestr: [],//需要转码
				cardidlist: []//勾选则购物车id,需要转码
			};
			var THIS = this;
			for (var n=0;n<this.car_list.length;n++){
				if(this.car_list[n].ischecked){
					updatestr.updatestr.push({
						id:this.car_list[n].id,
						count:this.car_list[n].buycount
					})
					updatestr.cardidlist.push(this.car_list[n].id);
				} 
			}
			...
			updatestr.updatestr = JSON.stringify(updatestr.updatestr);
			updatestr.cardidlist = updatestr.cardidlist + '';
			var w = plus.nativeUI.showWaiting(); 
			app.http_post('/mall/shopping/cart/settle/info',updatestr,function(res){
				if(res.code == 200){
					app.show_webview_fire('submit_order','loadinfo',{
						data:res.data,
						cardidlist:updatestr.cardidlist
					});
				}else{
					w.close()
				}
...
//			app.show_webview_fire('submit_order','loadinfo',{})
		...
		//切换选中
		select_goods:function(i){
			if(i == -1){
				for (var j=0;j<this.car_list.length;j++){
					this.car_list[j].ischecked = !this.selected_all;
				}
				this.selected_all = !this.selected_all;
			}
			if(i !== -1){
				this.car_list[i].ischecked = !this.car_list[i].ischecked;
			}
			this.count_select();
		},
		//计算选中的件数
		count_select:function(){
			...
			for (var n=0;n<this.car_list.length;n++){
				temp+=this.car_list[n].ischecked;
				if(this.car_list[n].ischecked){
					price+=this.car_list[n].allprice.intpart;
					price+=+('.'+this.car_list[n].allprice.floatpart);
					fee+=this.car_list[n].fee;
					selected_count+=+this.car_list[n].buycount;
				} 
			}
			if(temp == this.car_list.length){
		...
		update_amount:function(i,e,type){//购买数量
			var _self = this;
			if(type == 'plus'){
				_self.car_list[i].buycount++;
			}
			if(_self.car_list[i].buycount > 1 && type == 'subtract'){
				_self.car_list[i].buycount--;
			}
			var price = _self.car_list[i].buycount * e.price;
			_self.car_list[i].allprice.intpart = parseInt(price);
			try{
				var float_p = price.toFixed(2);
				_self.car_list[i].allprice.floatpart = float_p.substr(float_p.indexOf('.')).replace('.','');
				
			}catch(e){
				_self.car_list[i].allprice.floatpart = 00;
			}
			_self.count_select();
		},
		//删除购物车
		del_goods:function(){
			...
			for (var n=0;n<this.car_list.length;n++){
				if(this.car_list[n].ischecked){
					cardidlist.push(this.car_list[n].id)
				} 
			}
			app.http_get('/mall/shopping/cart/del?cardidlist='+cardidlist+'',function(res){
				if(res.code == 200){
					THIS.get_carlist();	
				}
			})
...
mui.ready(function(){
	...
	window.addEventListener('loadinfo',function(e){
		...
		if(e.detail.should_reset){
			container.reset()
		}
		setTimeout(function(){
			container.get_carlist();
		},200)
	})
	mui.back = function(){
		container.reset();
