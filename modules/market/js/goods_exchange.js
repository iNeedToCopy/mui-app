...
var container = new Vue({
	el: "#container",
	data:{
		should_hide: false,//是否需要隐藏详情页
		issubmit:false,
		isloading:false,
		type:'',
		orderinfo:{},//订单详情
		malldrawback_reason:[],//退款原因
		malldrawbackchange_reason:[],//退换原因
		exchange_type: '', //退换类型
		reason_type: '',//原因类型
		reason_detail: '',//具体原因
		imglist:[],//图片凭证
	...
	methods:{
		reset:function(){
			...
			this.type = '';
			this.orderinfo = {};//订单详情
			this.malldrawback_reason = [];//退款原因
			this.malldrawbackchange_reason = [];//退换原因
			this.exchange_type =  ''; //退换类型
			this.reason_type =  '';//原因类型
			this.reason_detail =  '';//具体原因
		},
		take_img:function(e){
			var _type = e.length + 1;
		...
		submit:function(){//提交
			if(this.type == '退换' && this.exchange_type == ''){
				mui.toast('还没有选择退换类型哦');
				return;
			}
			if(this.reason_type == ''){
				mui.toast('还没有选择原因哦');
			...
			var _data = {
				...
				applytype : this.exchange_type,
				masterid : this.reason_type,
				secondarycause : this.reason_detail,
				imglist : this.imglist + ''
			}
			var w = plus.nativeUI.showWaiting();
			if(this.type == '退款'){
				app.http_post('/mall/order/drawback',_data,function(res){
					w.close();
					mui.toast(res.desc);
					if(res.code == 200){
						THIS.issubmit = true;
			...
			if(this.type == '退换'){
				app.http_post('/mall/order/drawback/change',_data,function(res){
					w.close();
					mui.toast(res.desc);
					if(res.code == 200){
						THIS.issubmit = true;
...
mui.ready(function(){
    ...
    window.addEventListener('loadinfo',function(e){
    	...
    	container.orderinfo = e.detail.order_detail;
    	container.should_hide = e.detail.should_hide;
    	container.type = e.detail.type;
    	container.malldrawback_reason = JSON.parse(app.get_item('cause')).malldrawback;
    	container.malldrawbackchange_reason = JSON.parse(app.get_item('cause')).malldrawbackchange;
    ...
    mui.back = function(){
    	...
    	if(container.issubmit){
			...
			mui.fire(opener,'loadinfo',{
				action_type : container.type == '退款' ? 'nosend' : 'noreceive'
			})
    	}
    	if(container.should_hide){
			var detail = plus.webview.getWebviewById('market_order_detail');
			plus.webview.hide(detail,'slide-out-right',0);
    	}
    	container.reset();
