...
var container = new Vue({
	...
	methods:{
		...
		get_list:function(id){
			var THIS = this;
			app.http_get('/mall/order/comment?id=' + id,function(res){
				THIS.isloading = false;
				if(res.code == 200){
					res.data.goodsinfo.forEach(function(e,i){
						e.text = '';
						e.imglist = [];
					})
		...
		take_img:function(e){
			var _type = e.imglist.length + 1;
			image_uploader.upload('comment',function(data){
				if(data.code == 200){
					if(_type == 1)
						e.imglist.push(data.url);
					else if(_type == 2){
						e.imglist.push(data.url);
					}
					else if(_type == 3){
		...
		submit:function(){
			...
			this.$refs.goodsscore.forEach(function(e,i){
				THIS.orderinfo.goodsinfo[i].goodsscore = e.star;
			})
			this.$refs.sendscore.forEach(function(e,i){
				THIS.orderinfo.goodsinfo[i].sendscore = e.star;
			})
			this.orderinfo.goodsinfo.forEach(function(e,i){
				_data.commentstr.push({
					gid: e.gid,
					text: e.text,
					imglist: e.imglist.join(),
					goodsscore: e.goodsscore,
					sendscore: e.sendscore,
				})
			});
			_data.commentstr = JSON.stringify(_data.commentstr);
			app.http_post('/mall/order/commentSave',_data,function(res){
				w.close();
				if(res.code == 200){
					mui.toast(res.desc);
					THIS.issubmit = true;
...
mui.ready(function(){
	...
	window.addEventListener('loadinfo',function(e){
		...
		setTimeout(function(){
			container.get_list(e.detail.order_detail.orderid);
		},200)
	})
	mui.back = function(){
    	scrollTo(0,0);
