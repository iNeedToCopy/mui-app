...
mui("body").on('tap','.mui-backdrop',function(){
	console.log('点击了返回或者蒙版')
	var cuurent = plus.webview.currentWebview().id;
	if(cuurent == 'submit_order'){
		plus.webview.hide(cuurent);
		mui.toast('请尽快付款，超过24小时将自动取消');
...
var container = new Vue({
	...
	methods:{
		get_goods:function(id) {
			var THIS = this;
			app.http_get('/mall/shopping/cart/settle/goods?cardidlist='+id,function(res){
				if(res.code == 200){
		...
		goto_selectlocation: function() {//跳转选择收货地址
		...
		submit_order:function(){
			...
			for (var i=0;i<this.goodslist.length;i++) {
				_data.message.push({
					sid:this.goodslist[i][0].sid,
					message:this.goodslist[i].comment,
				})
			}
			_data.message = JSON.stringify(_data.message);
			var w = plus.nativeUI.showWaiting("正在获取收款信息...");
			var _self = this;
			app.http_post('/mall/order/submit',_data,function(ret){
				w.close();
				if(ret.code == 200){
					...
					plus.payment.request(hipay.wxpay, ret.data, function(res) { 
						app.json('调起支付成功'+res);
						if(res){
							app.http_get('/mall/order/pay/complete?id='+ret.desc,function(res){
								if(res.code == 200){
									mui.toast('支付成功');
...
mui.ready(function(){
	...
	window.addEventListener('loadinfo',function(e){
		...
		container.address = e.detail.data.address;
		container.allbenefit = e.detail.data.allbenefit;
		container.benefitlist = e.detail.data.benefitlist;
		container.paymoney = e.detail.data.paymoney;
		container.goodscount = e.detail.data.goodscount;
		container.cardidlist = e.detail.cardidlist;
		setTimeout(function(){
			container.get_goods(e.detail.cardidlist);
		},200)
		hipay.updateservice();
	});
	window.addEventListener('location_selected',function(e){
		plus.nativeUI.closeWaiting();
		e.detail.address.detail = e.detail.address.address;
